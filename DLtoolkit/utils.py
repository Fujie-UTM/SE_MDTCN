...
def select_best_hyperparams(
    ...
    返回：  
    best_model_params, best_opt_params
    """
...
class EEGDataset(Dataset):
    ...
    def __init__(self, EEGdata, EEGlabel):
        ...
        elif len(EEGdata.shape) == 3:  # 如果输入是 3D 数据
            # 扩展一个维度（作为通道维），再转为 float32 张量
            self.data = torch.tensor(np.expand_dims(EEGdata, axis=1), dtype=torch.float32)
        else:
            raise ValueError(f"EEGdata must be 3D or 4D, got shape {EEGdata.shape}")
            
        if len(EEGlabel) != len(self.data):
            raise ValueError("EEGdata and EEGlabel have inconsistent sample counts")
...
def parse_param_cell(x):
    """把 CSV 中的参数列还原为 dict。兼容 JSON 字符串和 Python 字面量字符串。"""
    ...
    if isinstance(x, str):
        for loader in (json.loads, ast.literal_eval):
            try:
                ...
                pass
    raise ValueError(f"Cannot parse parameter cell as dict: {x}")


def param_key(d):
    """生成稳定的参数字符串，便于比较、分组和写 CSV。"""
    return json.dumps(d, sort_keys=True, ensure_ascii=False)


def _combo_key(model_params_dict, opt_params_dict):
    """模型参数 + 优化器参数的唯一键。"""
    return param_key(model_params_dict) + " || " + param_key(opt_params_dict)


def event_labels(event_id):
    """按 event_id 的数值排序，返回显示标签和 confusion_matrix 使用的标签值。"""
    pairs = sorted(event_id.items(), key=lambda kv: kv[1])
    display_labels = [k for k, _ in pairs]
    class_values = [v for _, v in pairs]
    return display_labels, class_values


def read_confusion_matrix_cell(cell):
    """读取 CSV 中保存的 confusion matrix。"""
    if isinstance(cell, str):
        try:
            return np.array(json.loads(cell), dtype=int)
        except Exception:
            return np.array(ast.literal_eval(cell), dtype=int)
    return np.array(cell, dtype=int)


def prepare_param_index_and_folders(
    train_result_csv,
    test_dir,
    best_model_params,
    best_opt_params,
    ranked_params_csv=None,
    param_prefix="param",
):
    """
    读取 CV 网格搜索结果，为每组超参数分配 param_id，并创建对应的测试结果文件夹。

    该函数只负责“超参数索引与目录准备”，不负责测试，也不负责作图。

    参数
    ----
    train_result_csv : str | Path
        第 6 部分合并后的网格搜索结果 CSV，一般是 train_dir / "GSresults.csv"。
    test_dir : str | Path
        测试结果根目录。函数会在该目录下创建 param_001、param_002、... 文件夹。
    best_model_params : dict
        由交叉验证结果选出的最佳模型超参数。
    best_opt_params : dict
        由交叉验证结果选出的最佳优化器超参数。
    ranked_params_csv : str | Path | None
        第 6 部分导出的排序文件，一般是 train_dir / "params_sorted_GSresults.csv"。
        如果存在，则 param_001 对应 CV 排名第 1 的超参数；如果不存在，则从
        train_result_csv 中按 CV 指标重新汇总排序。
    param_prefix : str
        超参数文件夹前缀，默认生成 param_001、param_002、...

    返回
    ----
    train_results_df : pd.DataFrame
        增加了 model_params_dict、opt_params_dict、combo_key 等辅助列的训练结果表。
    param_index_df : pd.DataFrame
        每组唯一超参数的索引表，包含 param_id、cv_rank、param_dir 等信息。
    param_meta : dict
        combo_key -> param 信息的字典，测试循环中可用来快速定位 param_id 和 param_dir。
    """
    train_result_csv = Path(train_result_csv)
    test_dir = Path(test_dir)
    test_dir.mkdir(parents=True, exist_ok=True)

    if ranked_params_csv is None:
        ranked_params_csv = train_result_csv.parent / "params_sorted_GSresults.csv"
    ranked_params_csv = Path(ranked_params_csv)

    # 1) 读取每个 subject 的 CV 网格搜索结果，并生成稳定 key。
    train_results_df = pd.read_csv(train_result_csv)
    train_results_df["model_params_dict"] = train_results_df["model_params"].apply(parse_param_cell)
    train_results_df["opt_params_dict"] = train_results_df["opt_params"].apply(parse_param_cell)
    train_results_df["model_params_key"] = train_results_df["model_params_dict"].apply(param_key)
    train_results_df["opt_params_key"] = train_results_df["opt_params_dict"].apply(param_key)
    train_results_df["combo_key"] = train_results_df.apply(
        lambda r: _combo_key(r["model_params_dict"], r["opt_params_dict"]),
        axis=1,
    )

    # 2) 标记 CV-best 超参数。注意：这里只用 CV 结果得到的 best_*，不会用测试集指标反选。
    best_combo_key = _combo_key(best_model_params, best_opt_params)

    # 3) 优先使用第 6 部分导出的 CV 排序结果，让 param_001 对应 CV 最优超参数。
    if ranked_params_csv.exists():
        param_index_df = pd.read_csv(ranked_params_csv)
        param_index_df["model_params_dict"] = param_index_df["model_params"].apply(parse_param_cell)
        param_index_df["opt_params_dict"] = param_index_df["opt_params"].apply(parse_param_cell)
        param_index_df["model_params_key"] = param_index_df["model_params_dict"].apply(param_key)
        param_index_df["opt_params_key"] = param_index_df["opt_params_dict"].apply(param_key)
        param_index_df["combo_key"] = param_index_df.apply(
            lambda r: _combo_key(r["model_params_dict"], r["opt_params_dict"]),
            axis=1,
        )
    else:
        # 兜底：如果没有排序文件，就根据 train_results 汇总出唯一超参数组合。
        param_index_df = (
            train_results_df
            .groupby(["combo_key", "model_params_key", "opt_params_key"], as_index=False)
            .agg(
                mean_valid_acc=("mean_valid_acc", "median"),
                std_valid_acc=("std_valid_acc", "median"),
                mean_valid_loss=("mean_valid_loss", "median"),
                std_valid_loss=("std_valid_loss", "median"),
            )
            .sort_values(["mean_valid_acc", "std_valid_acc"], ascending=[False, True], kind="mergesort")
        )
        param_index_df["model_params_dict"] = param_index_df["model_params_key"].apply(json.loads)
        param_index_df["opt_params_dict"] = param_index_df["opt_params_key"].apply(json.loads)

    # 4) 分配 param_id、cv_rank 和文件夹路径。
    param_index_df = param_index_df.drop_duplicates("combo_key", keep="first").reset_index(drop=True)
    param_index_df.insert(0, "cv_rank", np.arange(1, len(param_index_df) + 1))
    param_index_df.insert(
        0,
        "param_id",
        [f"{param_prefix}_{i:03d}" for i in range(1, len(param_index_df) + 1)],
    )
    param_index_df["is_cv_best_hyperparams"] = param_index_df["combo_key"] == best_combo_key
    param_index_df["param_dir"] = param_index_df["param_id"].apply(lambda x: str(test_dir / x))

    # 5) 为每组超参数创建文件夹，并保存参数说明。
    for _, row in param_index_df.iterrows():
        param_dir = Path(row["param_dir"])
        param_dir.mkdir(parents=True, exist_ok=True)

        payload = {
            "param_id": row["param_id"],
            "cv_rank": int(row["cv_rank"]),
            "is_cv_best_hyperparams": bool(row["is_cv_best_hyperparams"]),
            "model_params": row["model_params_dict"],
            "opt_params": row["opt_params_dict"],
        }
        # 如果排序表中有 CV 指标，也一起写入。
        for col in ["mean_valid_acc", "std_valid_acc", "mean_valid_loss", "std_valid_loss"]:
            if col in row.index and pd.notna(row[col]):
                payload[col] = float(row[col])

        with open(param_dir / "params.json", "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)

        with open(param_dir / "README.txt", "w", encoding="utf-8") as f:
            f.write(
                "This folder contains test-set results for one hyperparameter combination.\n"
                "Hyperparameter selection is still based only on cross-validation results.\n"
                "Test-set metrics are saved for reporting/comparison, not for selecting hyperparameters.\n"
            )

    # 6) 建立 combo_key -> param 信息索引，并保存根目录索引文件。
    param_meta = param_index_df.set_index("combo_key").to_dict(orient="index")

    param_index_df.drop(columns=["model_params_dict", "opt_params_dict"], errors="ignore").to_csv(
        test_dir / "param_index.csv", index=False, encoding="utf-8"
    )

    return train_results_df, param_index_df, param_meta


def save_test_results_by_param(
    test_records,
    test_dir,
    param_index_df,
):
    ...
    输出内容：
    1. test_results_all_hyperparams.csv
       - 所有 subject × hyperparameter 的测试总表。

    2. test_results_best_params.csv
       - CV-best 超参数对应的测试结果。
       - 效果等同于原先的 test_results.csv，但文件名更清楚。

    3. test_param_summary_all_hyperparams.csv
       - 每组超参数的测试集汇总结果。

    4. param_index.csv
       - 超参数索引表，追加测试集汇总指标。

    5. cv_best_param_id.txt
       - 只记录 CV-best 对应的 param_id，例如 param_001。

    6. cv_best_param_marker.csv
       - CV-best 超参数的完整索引与汇总信息。

    7. param_xxx/test_results.csv
       - 每组超参数在所有 subject 上的测试结果。

    8. param_xxx/test_summary.csv
       - 每组超参数自己的测试汇总。
    """
    ...
    # ---------- 1. 保存所有 subject × hyperparameter 的测试总表 ----------
    test_results = pd.DataFrame(test_records)

    test_results.to_csv(
        test_dir / "test_results_all_hyperparams.csv",
        index=False,
        encoding="utf-8"
    )

    if test_results.empty:
        print("No test results were generated. Please check model_path and GSresults.csv.")
        return test_results, pd.DataFrame(), param_index_df

    # ---------- 2. 按 param_id 汇总所有超参数的测试结果 ----------
    param_test_summary = (
        test_results
        .groupby("param_id", as_index=False)
        .agg(
            cv_rank=("cv_rank", "first"),
            n_subjects=("sub_i", "nunique"),

            mean_test_acc=("acc", "mean"),
            std_test_acc=("acc", "std"),

            mean_test_f1_macro=("f1_macro", "mean"),
            std_test_f1_macro=("f1_macro", "std"),

            mean_test_kappa=("cohen_kappa", "mean"),
            std_test_kappa=("cohen_kappa", "std"),

            is_cv_best_hyperparams=("is_cv_best_hyperparams", "max"),

            mean_cv_valid_acc=("cv_mean_valid_acc", "mean"),
            mean_cv_valid_loss=("cv_mean_valid_loss", "mean"),
        )
        .sort_values(["cv_rank"], ascending=True, kind="mergesort")
    )

    # ---------- 3. 每个 param_xxx 文件夹内保存该超参数的测试结果 ----------
    for param_id, param_test_df in test_results.groupby("param_id", sort=False):
        param_dir = test_dir / param_id
        param_dir.mkdir(parents=True, exist_ok=True)

        # 逐 subject 测试结果
        param_test_df = param_test_df.sort_values("sub_i").copy()

        param_test_df.to_csv(
            param_dir / "test_results.csv",
            index=False,
            encoding="utf-8"
        )

        # 该超参数的测试汇总
        one_summary = param_test_summary.loc[
            param_test_summary["param_id"] == param_id
        ].copy()

        one_summary.to_csv(
            param_dir / "test_summary.csv",
            index=False,
            encoding="utf-8"
        )

    # ---------- 4. 根目录保存所有超参数的测试汇总 ----------
    param_test_summary.to_csv(
        test_dir / "test_param_summary_all_hyperparams.csv",
        index=False,
        encoding="utf-8"
    )

    # ---------- 5. 更新 param_index.csv，追加测试集汇总指标 ----------
    # 不 drop 任何列，保留 param_index_df 的完整信息。
    # 如果 param_index_df 和 param_test_summary 有同名列，
    # merge 后测试汇总表中的同名列会自动带上 _test_summary 后缀。
    param_index_out = param_index_df.merge(
        param_test_summary,
        on="param_id",
        how="left",
        suffixes=("", "_test_summary")
    )

    param_index_out.to_csv(
        test_dir / "param_index.csv",
        index=False,
        encoding="utf-8"
    )

    # ---------- 6. 单独保存 CV-best 超参数对应的测试结果 ----------
    test_results_best_params = test_results.loc[
        test_results["is_cv_best_hyperparams"].astype(bool)
    ].copy()

    test_results_best_params.to_csv(
        test_dir / "test_results_best_params.csv",
        index=False,
        encoding="utf-8"
    )

    # ---------- 7. 标出 CV-best 是哪个参数 ----------
    cv_best_params = param_index_out.loc[
        param_index_out["is_cv_best_hyperparams"].astype(bool)
    ].copy()

    if not cv_best_params.empty:
        cv_best_param_id = str(cv_best_params.iloc[0]["param_id"])

        # 简单文本标记：只记录最佳超参数 ID
        with open(test_dir / "cv_best_param_id.txt", "w", encoding="utf-8") as f:
            f.write(cv_best_param_id)

        # 保存完整 CV-best 参数信息
        cv_best_params.to_csv(
            test_dir / "cv_best_param_marker.csv",
            index=False,
            encoding="utf-8"
        )

        print(f"CV-best hyperparameter is: {cv_best_param_id}")
        print(f"Best-parameter test results saved to: {test_dir / 'test_results_best_params.csv'}")
        print(f"All hyperparameter test results saved under: {test_dir}")

    else:
        print("Warning: no CV-best hyperparameter was marked in param_index_df.")

    return test_results, param_test_summary, param_index_out


def save_subject_results_by_param(
    test_records,
    test_dir,
    param_index_df=None,
    subject_results_dirname="subject_results",
    param_col="param_id",
    sub_col="sub_i",
    cv_bacc_col="cv_mean_valid_acc",
    test_bacc_col="acc",
    out_cv_bacc_col="cv_valid_bacc_mean",
    out_test_bacc_col="test_bacc",
    overwrite=True,
):
    """
    按超参数编号保存逐 subject 的 CV validation bacc 均值和 test bacc。

    该函数会在 ``test_dir`` 下创建 ``subject_results`` 文件夹，并为每个
    ``param_id`` 生成一个同名 CSV 文件，例如：

        subject_results/param_001.csv
        subject_results/param_002.csv

    每个 CSV 固定包含三列：

        1. sub_i
        2. cv_valid_bacc_mean
        3. test_bacc

    参数
    ----
    test_records : list[dict] | pd.DataFrame
        测试记录。通常就是测试循环收集的 ``test_records``，或
        ``save_test_results_by_param`` 内部生成的 ``test_results``。
        需要至少包含 ``param_id``、``sub_i``、测试 bacc 列，以及 CV validation bacc 列。
    test_dir : str | Path
        测试结果根目录。函数会在该目录下创建 ``subject_results_dirname`` 文件夹。
    param_index_df : pd.DataFrame | None
        由 ``prepare_param_index_and_folders`` 返回的超参数索引表。
        若提供，则按其中的 ``param_id`` 顺序输出；若某个 param 没有测试记录，
        也会生成只有表头的空 CSV。
    subject_results_dirname : str
        逐 subject 结果文件夹名，默认 ``subject_results``。
    param_col, sub_col : str
        超参数编号列名和 subject 编号列名。
    cv_bacc_col : str
        交叉验证 validation bacc 均值所在列名，默认 ``cv_mean_valid_acc``。
        如果不存在，会按顺序尝试 ``mean_valid_acc``、``valid_bacc_mean``、
        ``cv_valid_bacc_mean`` 作为兜底。
    test_bacc_col : str
        测试集 bacc 所在列名，默认 ``acc``。如果不存在，会尝试
        ``test_bacc``、``balanced_accuracy``、``bacc`` 作为兜底。
    out_cv_bacc_col, out_test_bacc_col : str
        输出 CSV 中第 2、3 列的列名。
    overwrite : bool
        若为 False 且目标 CSV 已存在，则跳过该文件。

    返回
    ----
    subject_results_dir : Path
        保存逐 subject CSV 文件的文件夹。
    written_files : list[Path]
        本次写出的 CSV 文件路径列表。
    """
    test_dir = Path(test_dir)
    subject_results_dir = test_dir / subject_results_dirname
    subject_results_dir.mkdir(parents=True, exist_ok=True)

    test_results = pd.DataFrame(test_records).copy()

    # 即使没有测试记录，也尽量根据 param_index_df 建立空文件，方便后续流程检查。
    if param_index_df is not None and not pd.DataFrame(param_index_df).empty:
        param_order = pd.DataFrame(param_index_df)[param_col].astype(str).drop_duplicates().tolist()
    elif not test_results.empty and param_col in test_results.columns:
        param_order = test_results[param_col].astype(str).drop_duplicates().tolist()
    else:
        return subject_results_dir, []

    if test_results.empty:
        test_results = pd.DataFrame(columns=[param_col, sub_col, cv_bacc_col, test_bacc_col])

    required_base_cols = [param_col, sub_col]
    missing_base = [c for c in required_base_cols if c not in test_results.columns]
    if missing_base:
        raise KeyError(f"test_records 缺少必要列: {missing_base}")

    # 兼容不同阶段/旧代码里可能使用的列名。
    cv_candidates = [cv_bacc_col, "mean_valid_acc", "valid_bacc_mean", "cv_valid_bacc_mean"]
    test_candidates = [test_bacc_col, "test_bacc", "balanced_accuracy", "bacc"]

    cv_source_col = next((c for c in cv_candidates if c in test_results.columns), None)
    test_source_col = next((c for c in test_candidates if c in test_results.columns), None)

    if cv_source_col is None:
        raise KeyError(
            "test_records 中找不到交叉验证 validation bacc 均值列；"
            f"已尝试列名: {cv_candidates}"
        )
    if test_source_col is None:
        raise KeyError(
            "test_records 中找不到 test bacc 列；"
            f"已尝试列名: {test_candidates}"
        )

    # 避免同一 param-subject 有多条记录时写出重复行；保留最后一次结果。
    test_results[param_col] = test_results[param_col].astype(str)
    test_results = test_results.drop_duplicates([param_col, sub_col], keep="last")

    written_files = []
    for param_id in param_order:
        out_path = subject_results_dir / f"{param_id}.csv"
        if out_path.exists() and not overwrite:
            continue

        one_df = (
            test_results.loc[test_results[param_col] == str(param_id), [sub_col, cv_source_col, test_source_col]]
            .copy()
            .sort_values(sub_col, kind="mergesort")
        )
        one_df.columns = [sub_col, out_cv_bacc_col, out_test_bacc_col]

        one_df.to_csv(out_path, index=False, encoding="utf-8")
        written_files.append(out_path)

    print(f"Subject-level results saved to: {subject_results_dir}")
    return subject_results_dir, written_files



def plot_one_param_results(param_test_df, param_dir, param_id, mc_labels, chance_level):
    """为某一组超参数保存柱状图和组水平混淆矩阵。"""
    if param_test_df.empty:
        print(f"{param_id}: empty test results, skip plotting.")
        return

    plot_dir = Path(param_dir) / "report_drawing"
    plot_dir.mkdir(parents=True, exist_ok=True)

    # ---------- 1) Per-subject balanced accuracy bar plot ----------
    plot_df = param_test_df.copy().sort_values("sub_i")
    plot_df["acc_percent"] = plot_df["acc"] * 100

    mean_acc = plot_df["acc_percent"].mean()
    std_acc = plot_df["acc_percent"].std()
    ...
    ax.bar(x_pos, plot_df["acc_percent"].values)
    ax.set_xticks(x_pos)
    ax.set_xticklabels(plot_df["sub_i"].astype(str).values)
    ...
    ax.text(
        x=0.98, y=chance_level * 100 + 1,
        s=f"Chance={chance_level * 100:.1f}%",
    ...
    ax.set_title(f"{param_id}: Test Performance")
    fig.tight_layout()
    plt.show()
    fig.savefig(plot_dir / "group_classification_results.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # ---------- 2) Group confusion matrix ----------
    num_conds = len(mc_labels)
    BG_CM = np.zeros((num_conds, num_conds), dtype=int)

    for cell in plot_df["confusion_matrix"]:
        arr = read_confusion_matrix_cell(cell)
        if arr.shape != (num_conds, num_conds):
            raise ValueError(
                f"{param_id}: confusion matrix shape mismatch: "
                f"got {arr.shape}, expected {(num_conds, num_conds)}"
            )
        BG_CM += arr
    ...
    ax.set_title(f"{param_id}: Group Confusion Matrix")
    fig.tight_layout()
    plt.show()
    fig.savefig(plot_dir / "Group_Confusion_Matrix.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
