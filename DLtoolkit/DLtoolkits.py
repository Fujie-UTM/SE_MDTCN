...
def run_one_epoch( 
    ...
    注意：
        - train 模式下，epoch_acc 为普通 accuracy。
        - val/test 模式下，epoch_acc 为 balanced accuracy。
    ...
    if is_train:
        # 训练模式必须提供优化器
        assert optimizer is not None, "In train mode must provide optimizer"

        # 设置模型为训练状态，启用 dropout、batchnorm 更新等
        model.train()
    else:
        # 设置模型为评估状态，关闭 dropout、batchnorm 更新等
        model.eval()

    # 初始化累计损失、累计正确数、累计样本数
    total_loss, total_correct, total_samples = 0.0, 0, 0

    # 用于保存验证 / 测试模式下的真实标签和预测标签
    all_true, all_pred = [], []

    # 训练模式需要梯度；验证 / 测试模式不需要梯度
    ctx = torch.enable_grad() if is_train else torch.no_grad()

    with ctx:
        # 根据 show_progress 决定是否使用 tqdm 进度条
        iterator = (
        ...
        for step, (X, y_orig) in iterator:
            ...
            if is_train and mixup_alpha is not None and mixup_alpha > 0:
                # 生成 MixUp 后的输入，以及两组标签 y_a、y_b 和混合比例 lam
                mixed_X, y_a, y_b, lam = mixup_data(
                    X, y_orig, alpha=mixup_alpha, device=device
                )

                # 前向传播
                logits = model(mixed_X)

                # MixUp 损失：按照 lam 对两份标签的损失加权
                ...
                # MixUp 下的近似正确数：
                # 如果预测等于 y_a，按 lam 计入；
                # 如果预测等于 y_b，按 1 - lam 计入。
                soft_correct = (
                    preds.eq(y_a).float() * lam +
                    preds.eq(y_b).float() * (1 - lam)
                ).sum().item()
                ...
                if (y_orig.dtype == torch.long and y_orig.dim() == 1) or \
                   (y_orig.dtype == torch.float and y_orig.dim() == 1):
                    ...
                    # 转为 long 类型，适配 CrossEntropyLoss 等常见损失函数
                    true_labels = y_orig.long()

                    # 计算损失
                    loss = loss_fn(logits, true_labels)

                else:
                    # 情况 2：标签是二维 soft label / 概率分布，例如 [B, C]
                    ...
                    # 计算每一行的和，用于判断是否已经是概率分布
                    row_sums = y_orig.sum(dim=1)

                    # 如果每行和接近 1，则认为已经是概率分布；
                    # 否则使用 softmax 转为概率分布。
                    probs = y_orig if torch.allclose(
                        row_sums,
                        torch.ones_like(row_sums),
                        atol=1e-6
                    ) else F.softmax(y_orig, dim=1)

                    # 使用 soft label 计算损失
                    loss = loss_fn(logits, probs)

                    # soft label 下，取概率最大的类别作为真实类别
                ...
                # 当前 batch 的普通正确数
                batch_correct = preds.eq(true_labels).sum().item()
            ...
            if not is_train:
                # 验证 / 测试模式下保存真实标签和预测标签
                all_true.append(true_labels.cpu())
                all_pred.append(preds.cpu())

            if is_train and show_progress and (
                step % log_steps == 0 or step == len(dataloader)
            ):
                # 当前平均 loss
    ...
    if is_train:
        # 训练模式：保持原逻辑，epoch_acc 为普通 accuracy
        epoch_acc = total_correct / total_samples

    else:
        # 验证 / 测试模式：将所有 batch 的标签拼接起来
        all_true = torch.cat(all_true)
        all_pred = torch.cat(all_pred)

        # 计算 balanced accuracy
        # balanced accuracy = 每个类别 recall 的平均值
        #
        # 对每一个真实类别 c：
        # recall_c = 预测为 c 且真实为 c 的数量 / 真实为 c 的总数量
        #
        # 最后对所有类别的 recall 求平均。
        epoch_acc = torch.stack([
            (all_pred[all_true == c] == c).float().mean()
            for c in torch.unique(all_true)
        ]).mean().item()

    # 日志打印；变量名仍然保持 epoch_acc
    logging.info(
        f"{mode} epoch — loss: {epoch_loss:.4f}, acc: {epoch_acc:.1%}"
    )

    if is_train:
        # 训练模式返回 loss 和普通 accuracy
        return epoch_loss, epoch_acc

    else:
        # 验证 / 测试模式返回 loss、balanced accuracy、真实标签、预测标签
        return epoch_loss, epoch_acc, \
...
def EEG_train_validate_cv(
    ...
    Args:
        ...
        is_zscore_first (bool, optional): 是否在增强前进行 Z-score，默认 False。
...
def EEG_hyperparameter_grid_search_cv(
    ...
    Args:
        ...
        is_zscore_first (bool, optional): 是否在每折前执行折内 Z-score，默认为 False。
